...
static void setPllRate(uint32_t rate)
{
	...
	if ((curr_src == SYSCON_PLLCLKSRC_RTC) || (curr_src == SYSCON_PLLCLKSRC_WDTOSC)) {
...
static void get_option(void) {
	...
	while (1) {
		...
		if (ch == '1') {
			...
			if ((curr_src == SYSCON_PLLCLKSRC_WDTOSC) || (curr_src == SYSCON_PLLCLKSRC_RTC)) {
				fmode = 0;
			}
			break;
		}
			...
			fmode = 1 - fmode;
			if (fmode && ((curr_src == SYSCON_PLLCLKSRC_RTC) || (curr_src == SYSCON_PLLCLKSRC_WDTOSC))) {
