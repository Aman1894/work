...
/* Multi rate timer interrupt service routine */
void MRT_IRQHandler(void)
{
	...
	if (int_pend & (MRTn_INTFLAG(2))) {
		setupMRT(2, MRT_MODE_ONESHOT, 5);	/* Will fire in (1/5) seconds */
		Board_LED_Toggle(1);
		t2++;
	}
...
int main(void)
{
	int loop = 1;	/* Used to avoid unreachable statement */
	int mrtch;
